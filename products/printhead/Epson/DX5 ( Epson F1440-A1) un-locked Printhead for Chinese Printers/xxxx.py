...
# Updated Chart Function with rotated labels as default
def add_bar_chart(worksheet, title, position, cat_col, data_col, min_row, max_row, num_categories):
    ...
    # Format category axis (bottom) - rotate labels to 45 degrees
    chart.x_axis.title = "Categories"
    chart.x_axis.tickLblPos = "low"
    
    # Set text properties for rotated labels
    try:
        # Create text properties for label rotation
        rtp = RichTextProperties()
        rtp.rot = -450000  # Rotation in 1/60000th of a degree (-45 degrees)
        chart.x_axis.txPr = rtp
    except:
        # Fallback if the above doesn't work
        chart.x_axis.txPr = RichTextProperties(rot=-450000)
...
def add_bar_chart(worksheet, title, position, cat_col, data_col, min_row, max_row, num_categories):
    ...
    # Set text properties for rotated labels
    body_pr = TextBodyProperties(rot=-450000)  # -45 degrees
    rt = RichText(bodyPr=body_pr)
