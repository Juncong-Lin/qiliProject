...
def format_price_from_data(lower, higher):
    """Formats price from lower and higher values."""
    if lower is None or higher is None:
        return "Price not available"
    if lower == higher:
        return f"USD:{lower / 100:.2f}"
    else:
        return f"USD:{lower / 100:.2f}-{higher / 100:.2f}"

def detect_changes(new_data, old_data):
    """Detects changes between new and old product data."""
    changes = []
    if new_data['name'] != old_data['name']:
        changes.append(f"name changed from '{old_data['name']}' to '{new_data['name']}'")
    old_price_str = format_price_from_data(old_data['lower_price'], old_data['higher_price'])
    new_price_str = format_price_from_data(new_data['lower_price'], new_data['higher_price'])
    if old_price_str != new_price_str:
        changes.append(f"price changed from {old_price_str} to {new_price_str}")
    return changes
...
def scrape_product_details(product_url, session, output_dir, script_name, brands_list, current_product_number, total_products_found, retry_count=0):
    ...
    try:
        ...
        return {
            ...
            'brand': brand,
            'url': product_url  # Added URL for unique identification
        }, retry_count
...
def main():
    ...
    # Load previously scraped products for display
    ...
    with requests.Session() as session:
        while product_tasks:
            ...
            if product_data:
                ...
                # Check for changes or new product
                if product_data['url'] in previous_products_by_url:
                    old_data = previous_products_by_url[product_data['url']]
                    changes = detect_changes(product_data, old_data)
                    if changes:
                        message = f"Product {product_data['name']} ({product_data['url']}): " + ", ".join(changes)
                        change_messages.append(message)
                    # Update existing product data
                    for i, p in enumerate(products_by_brand[brand_key]):
                        if p['url'] == product_data['url']:
                            products_by_brand[brand_key][i] = product_data
                            break
                else:
                    # New product
                    message = f"New product: {product_data['name']} ({product_data['url']})"
                    change_messages.append(message)
    ...
    # Update xxxx.md with all current products and changes
    all_current_products = set()
    for products in products_by_brand.values():
        for product in products:
            all_current_products.add(product['name'])

    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(f"# Scraped Products\n\n")
        f.write(f"Last updated: {current_time}\n\n")
        f.write(f"Total products: {len(all_current_products)}\n\n")
        for product_name in sorted(all_current_products):
            f.write(f"- {product_name}\n")

    # Append changes and new products to xxxx.md
    if change_messages:
    ...
    # Display summary and changes in terminal
    ...
    if change_messages:
        ...
        print("\nNo changes or new products found.")
