# Save this file as xxxx.py (e.g., material_scraper.py)
# The output folder will be named 'xxxx' (e.g., 'material_scraper') in the same directory as this script
...
BRANDS = [
    'Roll_To_Roll_Style', 'Hoson', 'Without_Cable_Work', 'UV_Flatbed', 'Roland', 'Epson', 'Mimaki', 'Mutoh', 'Witcolor', 
    'Galaxy', 'Human', 'Gongzheng', 'Flora', 'Xuli', 'Xenos', 'Yongli', 'Zhongye', 'Skyjet', 'Yaselan', 'Teckwin', 
    'Ricoh', 'TitanJet', 'Seiko', 'Skycolor', 'Myjet', 'Phaeton', 'Meitu', 'JHF', 'Kingfisher', 'Liyu', 'Locor', 
    'Infiniti_Challenger', 'Handtop', 'FunsunJet', 'Ecotech', 'Encad', 'DGI', 'Docan', 'Crystaljet', 'Atexco', 
    'Audley', 'Allwin', 'Photo_Papers', 'Mesh', 'Adhevie_Vinyl', 'Oneway_Vision', 'Flex_banner', 'ink'
]
...
def scrape_product_details(product_url, session, output_dir, script_name, brands_list, retry_count=0):
    """Scrapes a single product detail page and returns product data."""
    try:
        print(f"  - Scraping product page: {product_url} (Attempt {retry_count + 1}/{MAX_RETRIES})")
...
def main():
    ...
    with requests.Session() as session:
        while tasks:
            task_type, url, retry_count = tasks.pop(0)
            ...
            if task_type == 'listing':
                ...
                try:
                    ...
                    for box in product_boxes:
                        ...
                        if link_tag and link_tag.get('href'):
                            ...
                            # Check product name to skip previously scraped products
                    ...
                    tasks.append((task_type, url, retry_count + 1))
                    time.sleep(5)
                ...
                if product_data:
                    ...
                    # Check for duplicates by product ID to be safe
